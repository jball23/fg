...
export const Cta = ({ image, heading, subHeading, buttonText }) => {
  return (
    <div className="flex justify-end bg-pink-500 px-4 pb-4 relative">
      <div className="bg-white p-4 bg-opacity-20 text-white flex flex-col md:flex-row items-center w-full md:w-auto text-center md:text-left rounded shadow-lg relative z-0">
        ...
        <div className="block md:mr-12 mb-4 md:mb-0">
          <p className="font-sans text-3xs tracking-widest font-400 uppercase opacity-50">{heading}</p>
          <p className="font-sans uppercase tabular-nums font-700 md:text-xl text-sm">{subHeading}</p>
        </div>
        <div className="md:ml-auto w-full md:w-auto">
          <a className="lg:px-6 py-3 px-4 group flex items-center justify-center w-full shadow-lg lg:text-base text-2xs uppercase duration-300 cursor-pointer font-600 transform text-gray-800 hover:text-black bg-white" href="https://opensea.io/collection/beanzofficial" target="_blank" rel="noreferrer">
